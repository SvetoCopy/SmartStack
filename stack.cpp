...
int StackVerify(Stack* stk) {
	...
	if (stk->hash_sum != GavGavHash(stk)) SetError(&error, HASH_ERROR);
...
int StackCtor_(Stack* stk, size_t capacity, const char* obj_name, const char* func, size_t line, const char* file) {
	...
	*((Canary_t*)(stk->data + stk->capacity)) = 0xDEADDEAD;
...
int StackRealloc(Stack* stk, size_t capacity) {
	...
	stk->data = (Elem_t*)realloc((char*)stk->data - sizeof(Canary_t), capacity * sizeof(Elem_t) + 2 * sizeof(Canary_t));
